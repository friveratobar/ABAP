...
& Funcional   *****                    
& Fecha       23052014                                             
& Objetivo    Muestra Reporte de Maquila - Finalización Trimestral   
&~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
REPORT  zmaqr001.
...
FORM get_data.
  ...
  DATA ls_retbapi    TYPE bapiret2.
  DATA ls_molib      TYPE zmm_molib_001.
  DATA ls_zqme015    TYPE zqme015.
  DATA lv_smbln      TYPE mseg-smbln.
  DATA lv_ebeln      TYPE ekko-ebeln.

  DATA lt_insppoi    TYPE STANDARD TABLE OF bapi2045l4.
  DATA lt_requeri    TYPE STANDARD TABLE OF bapi2045d1.
  DATA lt_results    TYPE STANDARD TABLE OF bapi2045d3.
  DATA lt_class      TYPE STANDARD TABLE OF sclass.
  DATA lt_objectdata TYPE STANDARD TABLE OF clobjdat.

  DATA wa_insppoi    LIKE LINE OF lt_insppoi.
  DATA wa_requeri    LIKE LINE OF lt_requeri.
  DATA wa_results    LIKE LINE OF lt_results.
  DATA wa_objectdata LIKE LINE OF lt_objectdata.
...
~~ Se extraen las entregas que cumplen las condiciones
  ...
  IF gt_likp IS NOT INITIAL.
    ...
    SELECT  ps~vbeln           Entrega
            ...
            ls~prueflos        Nº lote inspección
      ...
      FROM  lips  AS ps
            ...
            INNER JOIN qals AS ls
                    ON ls~charg       EQ ps~charg
                   AND ls~lagortchrg  EQ ps~lgort
             INNER JOIN mchb AS b
                     ON  b~charg      EQ ps~charg
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
~~ Valida el estatus del lote de inspección

    REFRESH t_system , t_user.
    CALL FUNCTION 'BAPI_INSPLOT_GETSTATUS'
      EXPORTING
        number        = wa_salida-prueflos
        language      = ls_langu
      TABLES
        system_status = t_system
        user_status   = t_user.

 Eliminamos los lotes anulados.
    CLEAR ls_system.
    READ TABLE t_system INTO ls_system WITH KEY sys_status = 'I0224'.
    IF sy-subrc EQ 0.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
~~ Analisís en Molymet + Numero de BAGS
...
~~ Analisís de Inspeccion
      ...
      CALL FUNCTION 'BAPI_INSPOPER_GETDETAIL'
        ...
        IMPORTING
          return                  = ls_retbapi
        TABLES
          insppoints              = lt_insppoi
          char_requirements       = lt_requeri
          sample_results          = lt_results.


      IF ls_retbapi-type NE 'E' AND lt_results IS NOT INITIAL.
        CLEAR  gv_indice.
        LOOP AT gt_elem_grup INTO wa_elem_grup WHERE ngrup LT 500.
          LOOP AT gt_elem INTO wa_elem WHERE ngrup EQ wa_elem_grup-ngrup.
            ADD 1 TO gv_indice.

            READ TABLE lt_requeri INTO wa_requeri WITH KEY mstr_char = wa_elem-carac.
            IF sy-subrc IS INITIAL.

              READ TABLE lt_results INTO wa_results WITH KEY  inspchar  = wa_requeri-inspchar
                                                              inspoper  = wa_requeri-inspoper
                                                              insplot   = wa_requeri-insplot.
              IF sy-subrc IS INITIAL.
                CLEAR lv_name.
                CONCATENATE 'WA_SALIDA-ELEM_' gv_indice INTO lv_name.
                ASSIGN (lv_name) TO fs1.

                TRANSLATE wa_results-mean_value USING ',.'.
                fs1   = wa_results-mean_value.
...
--- LOTE DE INSPECCIÓN
  CLEAR  wa_fcat.
  wa_fcat-fieldname   = 'PRUEFLOS'.         wa_fcat-just        = 'C'.
  wa_fcat-scrtext_m   = 'Lot. Insp'.        wa_fcat-tooltip     = 'Lote de Inspección'.
  wa_fcat-no_zero     = 'X'.
  APPEND wa_fcat TO gt_fcat.
...
--- Mo delivery accumlation (kg)
  ...
  wa_fcat-scrtext_m   = 'Mo Deliv.Accum(kg)'. wa_fcat-tooltip     = 'Mo Delivery Accum (kg)'.
  APPEND wa_fcat TO gt_fcat.
